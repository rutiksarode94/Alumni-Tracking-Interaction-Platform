<?php
    ...
    $crud = new Action();
    $chats = $crud->get_chats($_GET['gr_no']);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    <input type="hidden" name="alumni_id" id="alumni_id" value="<?php echo $_GET['gr_no']; ?>" />
...
?>
        <div style="clear: both;">
            <p style="float: <?php echo $_GET['gr_no'] == $val['receiver_gr_no'] ? "right" : "left"; ?>"><?php echo $val['message']; ?></p>
        </div>
<?php
    }
}
...
$.ajax({
    ...
    error: function() {
        ...
        function sendMessage() {
            var receiver_gr_no = $("#alumni_id").val();
            var message = $('#chat-message-input').val();

            $.ajax({
                url: "admin/send_chat.php",
                type: "post",
                data:{
                    receiver_gr_no: receiver_gr_no,
                    message: message
                },
                success:function(response) {
                    if(response) {
                        ...
                        $('#chat-messages').append(messageText + fileText);
